...
product_data.forEach(function(item) {
    ...
    product_item += '<td><input type="number" id="quantity_' + item.product_id + '" value="1" style="width: 90%; display: block; margin: 0 auto;"><button type="button" style="width: 100%;" id="add_to_cart_' + item.product_id + '">Th\xeam v\xe0o giỏ</button></td>';
    product_item += "</tr>";
...
if (add_to_cart.length > 0) for(var i = 0; i < add_to_cart.length; i++)add_to_cart[i].onclick = function() {
...
function get_product(id) {
    ...
    product_data.forEach(function(data) {
        if (data.product_id == id) result = data;
    });
    return result;
}
function renderCart() {
...
`;
        ...
        cart_data.forEach(function(cart_item) {
            ...
            <td><input type="number" class="quantity" data-id="` + cart_item.product_id + `" value="` + cart_item.quantity + `"></td>
            <td>` + amout + `</td>
        ...
        deleteCart();
        deleteAll();
    } else document.querySelector("#cart_data").innerHTML = "Giỏ h\xe0ng kh\xf4ng c\xf3 sản phẩm";
}
function updateCart() {
            ...
            cart_arr.forEach(function(cart_item) {
                ...
                cart_data.forEach(function(value, key) {
                    if (product_id == value.product_id) {
                        if (quantity_value > 0) cart_data[key] = {
                            product_id: product_id,
                            quantity: parseInt(quantity_value)
                        };
                        else cart_data.splice(key, 1);
                    }
                });
                var cart_json = JSON.stringify(cart_data);
                sessionStorage.setItem("cart", cart_json);
    ...
    document.querySelectorAll(".delete-item").forEach(function(del_item) {
        del_item.onclick = function() {
            if (confirm("Are you sure?")) {
                var product_id = this.parentElement.parentElement.querySelector(".quantity").getAttribute("data-id");
                product_id = parseInt(product_id);
                var cart_data = sessionStorage.getItem("cart");
                cart_data = JSON.parse(cart_data);
                cart_data.forEach(function(value, key) {
                    if (product_id == value.product_id) cart_data.splice(key, 1);
                });
                var cart_json = JSON.stringify(cart_data);
                sessionStorage.setItem("cart", cart_json);
